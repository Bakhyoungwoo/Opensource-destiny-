...
# 가장 유사한 옷 이름을 찾음
similar_cloth_name = find_similar_cloth_name(cloth_name, df['title'].tolist())

# title열에서 옷 이름이 일치하는 행을 찾음
matching_rows = df.loc[df['title'].str.lower() == similar_cloth_name.lower()]
...
if len(prices) == 0:
    ...
    print(f"{similar_cloth_name}의 가격은 {prices}원이며, 평균 가격은 {average_price:,}원, 가장 싼 가격은 {min_price:,}원 입니다.") #가독성을 위해 가격 출력 시 : ,를 사용하여 천 단위로 쉼표를 출력
    ...
    for index, row in df.iterrows():
        ...
        if abs(price_num - average_price) <= price_range and row['title'].lower() != similar_cloth_name.lower():
    ...
    if len(cloth_colors) > 0:
        same_color_clothes = df.loc[(df['title'].str.lower() != similar_cloth_name.lower()) & (df['color'].str.lower().isin(cloth_colors))]  # 색깔이 같은 옷 추출
