...
form_class1, _ = uic.loadUiType("PyQt5_ui/first_window.ui")
form_class2, _ = uic.loadUiType("PyQt5_ui/situation_second_window.ui")
form_class3, _ = uic.loadUiType("PyQt5_ui/situation_third_window.ui")
form_class4, _ = uic.loadUiType("PyQt5_ui/price_second_window.ui")
form_class5, _ = uic.loadUiType("PyQt5_ui/price_third_window.ui")
form_class6, _ = uic.loadUiType("PyQt5_ui/color_second_window.ui")
form_class7, _ = uic.loadUiType("PyQt5_ui/size_second_window.ui")



class total_window(QMainWindow, form_class1):
    # Get the current script directory
    script_dir = os.path.dirname(os.path.abspath(__file__))

    # Construct the relative path to the "Feature_gui_PyQt" folder
    relative_path = os.path.join(script_dir, "pythonProject2")

    try:
        os.chdir(relative_path)
        print(os.getcwd())  # Print the new current working directory
    except:
        print("Failed to change the directory.")
    def __init__(self):
        super().__init__()
        self.setupUi(self)
        self.pushButton_situation.clicked.connect(self.button_clicked_situation)
        ...
        self.pushButton_size.clicked.connect(self.button_clicked_size)
    ...
    # 버튼이 클릭될 때 새로운 창 생성
    def button_clicked_situation(self):
        self.close()
        self.situation_second_window = Situation_SecondWindow(self)
        self.situation_second_window.show()

    def button_clicked_size(self):
        self.close()
        self.size_second_window = Size_SecondWindow(self)
        self.size_second_window.show()

class Size_SecondWindow(QMainWindow, form_class7):
    def __init__(self, parent=None):
        super(Size_SecondWindow, self).__init__(parent)
        self.setupUi(self)
        self.pushButton_total.clicked.connect(self.button_clicked)
...
class Color_SecondWindow(QMainWindow, form_class6):
    def __init__(self, parent=None):
        ...
        self.pixmap = QPixmap('png/color_combinate.PNG')
    ...
    def tell_result_color(self):
        pixmap = QPixmap('png/color_combinate.PNG')
